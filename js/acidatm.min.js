function render(){let n="";for(let e=0;e<config.y;e++){let i="",t=e-config.my-1;if(e>config.my&&t<config.content.length-config.offset){for(let n=0;n<config.mx+1;n++)i+=o(n,e);i+=config.content[t+config.offset].innerText.length>0?config.content[t+config.offset].innerHTML:"";for(let n=config.mx+config.content[t+config.offset].innerText.length;n<config.x;n++)i+=o(n,e)}else for(let n=0;n<config.x;n++)i+=o(n,e);n+=i+"<br>"}function o(n,o){let e=.5*(noise.simplex3(n*config.grain,o*config.grain,config.steps*config.speed)+1);return config.symbols[Math.floor(e*config.symbols.length)]}config.steps++,config.container.innerHTML=n,setTimeout(function(){requestAnimationFrame(render)},100)}function calcMargins(){config.x=Math.ceil(window.innerWidth/3),config.y=Math.ceil(window.innerHeight/8),config.mx=Math.floor(.01*window.innerWidth),config.my=Math.floor(.005*window.innerHeight)}function seed(){config.speed=.1*Math.random()+.005,config.grain=.05*Math.random()+.01;let n=[];for(i=0;i<config.pool.length;i++)n.push(config.pool[i]);config.symbols=[" "];let o=1+Math.floor(Math.random()*Math.min(config.maxsymbols,n.length-2));for(let e=0;e<o;e++){let o=Math.floor(Math.random()*n.length);config.symbols.push(n[o]),n.splice(o,1)}}window.config={steps:0,speed:1,grain:1,pool:["-",".",":","*","$","+","!"],symbols:[" "],maxsymbols:3,content:[],container:!1,scroll:0,offset:0},window.addEventListener("load",function(){seed(),noise.seed((new Date).value);let n=document.getElementById("content");config.content=[].slice.call(n.getElementsByTagName("li")),config.maxOffset=config.content.length,n.remove(),config.container=document.createElement("div"),config.container.id="container",config.container.addEventListener("click",function(){seed()}),document.body.appendChild(config.container),document.addEventListener("wheel",function(n){config.scroll=Math.max(0,Math.min(config.scroll+.01*n.deltaY,config.maxOffset)),config.offset=Math.floor(config.scroll)}),calcMargins(),window.addEventListener("resize",function(){calcMargins()}),render()});
