...
function render(){
  ...
  function getNoise(x,y){
    let n = (noise.simplex3(x*config.grain,y*config.grain,config.steps * config.speed) + 1) * 0.5;
    return config.symbols[Math.floor(n * config.symbols.length)]
  }
  config.steps++
  config.container.innerHTML = output
  setTimeout(function () {
...
function seed(){
  ...
  config.symbols = [' ']
  let sn = 1 + Math.floor(Math.random() * Math.min(config.maxsymbols,(p.length - 2)))
  for(let i = 0; i < sn; i++){
    let s = Math.floor(Math.random() * p.length);
    config.symbols.push(p[s])
    p.splice(s, 1)
  }
}
